...
class MenuModule:
        ...
        if self._request.user.is_authenticated:
            ...
            if self._request.method == 'GET':
                ...
                if self._request.session.get('group_id'):
                    ...
                    group = data['group_list'].get(id=self._request.session['group_id'])
                    data['group'] = group
    ...
    def __get_menu_list(self, user: User, group: Group):
        ...
        # obtiene la lista de menus de tal forma que no se repitan
        # Usar un enfoque compatible con SQLite
        seen_menus = set()
        menu_unicos = []
        for gmp in group_module_permission_list:
            menu_id = gmp.module.menu_id
            if menu_id not in seen_menus:
